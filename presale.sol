 ...
 */
 contract Presale is Owned{
    ...
    uint256 public minAvaxLimit = 300 * (10 ** 18);
    uint256 public maxAvaxLimit = 600 * (10 ** 18);
    ...
    constructor(uint256 _rate, address _wallet, ERC20 _pToken, uint _closingTime, uint _vestedTime) {
        require(_rate > 0);
    ...
    function setMinAvaxLimit(uint256 amount) external onlyOwner {
        minAvaxLimit = amount;    
    }
    ...
    function setMaxAvaxLimit(uint256 amount) external onlyOwner {
        maxAvaxLimit = amount;    
    }
    ...
    // allows buyers to put their avax to get some pToken once the presale will closes
    receive() external payable{
        require(isPresaleOpen, "Presale is not open yet");
        require(isList == false || whiteListed[msg.sender] == true);
        require(msg.value >= minAvaxLimit, "You need to sell at least some min amount");
        require(paidTotal[msg.sender] + msg.value <= maxAvaxLimit);
        // safe transferFrom of the payout amount
        payable(wallet).transfer(msg.value);

        // calculate pToken amount to be created
        uint256 pTokenAmount = msg.value.mul(rate).div(RATE_DECIMALS);

        // update state
        avaxRaised = avaxRaised.add(msg.value);
        ...
        preBuys[msg.sender].avaxAmount = preBuys[msg.sender].avaxAmount.add(msg.value);
        paidTotal[msg.sender] += msg.value;
        emit TokenPurchase(
            msg.sender,
            msg.value,
            pTokenAmount
    ...
    function getClaimableAmount(address _account) public view returns (uint) {
        ...
        uint totalpTokenToClaim = preBuys[_account].avaxAmount.mul(rate).div(RATE_DECIMALS).mul(percentReleased).div(VESTING_TIME_DECIMALS);
    ...
    function claim() public {
        ...
        uint totalpTokenToClaim = preBuys[msg.sender].avaxAmount.mul(rate).div(RATE_DECIMALS).mul(percentReleased).div(VESTING_TIME_DECIMALS);
